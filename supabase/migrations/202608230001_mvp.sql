...
begin
  select space_id, visibility, created_by, owner_user_id
    ...
    from public.items where id = target_item_id;
...
create policy item_member_update on public.items for update using (
  public.is_space_member(space_id, auth.uid()) and (visibility = 'shared' or created_by = auth.uid() or owner_user_id = auth.uid() or public.is_space_admin(space_id, auth.uid()))
) with check (
  public.is_space_member(space_id, auth.uid()) and (visibility = 'shared' or created_by = auth.uid() or owner_user_id = auth.uid() or public.is_space_admin(space_id, auth.uid()))
);
...
create policy notification_event_self on public.notification_events for select using (recipient_user_id = auth.uid());
...
create policy item_photo_storage_select on storage.objects for select using (
  bucket_id = 'item-photos' and exists (
    ...
    where p.storage_path = name and public.is_space_member(i.space_id, auth.uid())
